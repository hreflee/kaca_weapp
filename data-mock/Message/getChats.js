/**
  ...
  getChats  获取聊天信息  POST  服务器需自行从session中获取当前用户微信id
    @param timeFrom  "yyyy-MM-dd HH:mm:ss" 上次拉取消息的时间即需要返回该时间点后的所有消息. 若该项留空则返回当前用户所有消息

    @return 聊天信息数组, 数组成员结构如示例, 假设当前用户的微信id为"A"
...
[
  {
    ...
    "messages": [            // 消息数组,包含发送和收到的消息. 按照时间顺序由远及进
      {
        "from": "A",           // 该条消息发送方openid, 这是一条当前用户向"B"发送的消息
        "to": "B",             // 该条消息接收方
      ...
      {
        "from": "B",           // 该条消息发送方openid, 这是一条"B"向当前用户发送的消息
        "to": "A",             // 该条消息接收方
